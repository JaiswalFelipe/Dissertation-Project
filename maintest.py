...
Original file is located at
    https://colab.research.google.com/drive/1pb5YvQqWhhfQqAEVf2mGVjCQwXq3ftmK
"""
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(validation_loader):
            ...
            inps, labs, cur_maps, xmins, ymaxs, xmaxs, ymins = data    
               ...
               #imshow(labs[0], normalize=False)
               #imshow(labs[1], normalize=False)
               #imshow(labs[2], normalize=False)
               #imshow(labs[3], normalize=False)
            
            #print("What is i?", i) # 0
            #print("data length", len(data)) # 7
            #print("inps shape", inps.shape) # torch.Size([4, 23, 250, 250])
            ...
            #print("outs shape", outs.shape) # torch.Size([4, 2, 250, 250])
            #print("outs[0].shape?",outs[0].shape) #  torch.Size([2, 250, 250])
            
            soft_outs = F.softmax(outs, dim=1) # softmax calcs on channels
            #print("soft outs shape before forloop", soft_outs.shape) # torch.Size([4, 2, 250, 250])
            ...
            for j in range(outs.shape[0]):
                ...
                #print("cur_maps after forloop j", cur_maps) # ([1,1,1,1])
                
                
                soft_outs_p = soft_outs.permute(0, 2, 3, 1).cpu().detach().numpy()
                #print("soft_outs_p on forloop", soft_outs_p.shape) #(4, 250, 250, 2)
                ...
                #print("prob_im unique vals after", prob_im)
