...
Original file is located at
    https://colab.research.google.com/drive/1LyK-UUoC4xcHqHYe2o24YYFVTwd2NUkb
"""
...
class NGValid(data.Dataset):
  def __init__(self, img_dir, mask_dir, whole_mask_dir, crop_size, output_path):
    ...
    self.whole_mask_dir = whole_mask_dir
    self.whole_mask = os.listdir(whole_mask_dir)
    ...
    print("orginal mask shape:",self.org_mask.shape,
          "n_dimensions:",self.data.ndim, 
          "image shape:",self.data.shape, 
          "first entry:",self.data[0].shape, 
          "min:",np.min(self.data),  
          "max:",np.max(self.data),
          "masks shape:",self.labels.shape, 
          "labels bin count",np.bincount(self.labels.astype(int).flatten()))
    ...
    print('num_channels and labels:', self.num_channels, self.num_classes, 
          "labels bin count after conversion of negative classes", np.bincount(self.labels.flatten()))
  ...
  def load_images(self):
        ...
        for wmask in self.whole_mask:
            whole_mask = imageio.imread(os.path.join(self.whole_mask_dir, wmask + '')).astype(int) 

            wmasks.append(whole_mask)
        
        return np.asarray(images), np.asarray(masks), np.asarray(coords), np.asarray(wmasks)
